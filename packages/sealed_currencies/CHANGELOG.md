...
- Finalized base classes (`FiatCurrency`, `NaturalLanguage`, etc.) instances - sealed classes with private constructors, you can create your own instances via `FiatCustom` for example:

```dart
const custom = FiatCustom(code: "XTL", codeShort: "XT"); // Custom currency.
```
...
- Added new static method to `FiatCurrency` - `maybeFromAnyCode`, which works same way as `fromAnyCode`, but doesn't throws `StateError`s if no currency object is found.
