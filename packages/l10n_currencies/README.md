...
- **Do you have LLM-agents instructions?**: Yes, for sure, under this spoiler,
  ...
  The package uses `CurrenciesLocaleMapper` to provide localized currency names. Note that mapper instances are designed for **single-use only** and clear their internal data after `localize()` is called.
