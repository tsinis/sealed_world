...
- **Dedicated Badge & Emblem Painters**:
  - Added 12 dedicated badge painters replacing generic shield fallbacks: Anguilla (`AIA`), Andorra (`AND`), Bermuda (`BMU`), Ecuador (`ECU`), Fiji (`FJI`), Falkland Islands (`FLK`), British Indian Ocean Territory (`IOT`), Jersey (`JEY`), Montserrat (`MSR`), Pitcairn (`PCN`), South Georgia (`SGS`), and Turks and Caicos (`TCA`).
  - Added missing emblem painters for Gibraltar (`GIB`), Dominica (`DMA`), Zimbabwe (`ZWE`), Cayman Islands (`CYM`), and Kyrgyzstan (`KGZ`), including related currency and language dual flags (`GIP`, `ZWG`, etc.).
- **Proportional Bounds**: Added `CustomElementsPainter.proportionalBounds` to preserve fixed aspect ratio artwork during flag scaling.

IMPROVEMENTS

- **Rendering Performance**:
  - Reduced draw calls across the catalogue by ~35% and clips by ~29%, removing all offscreen `saveLayer` passes.
...
- **Flag Artwork & Accuracy**:
  ...
  - Removed `isSimplified: true` from `AIA`, `FJI`, `JEY`, `MSR`, `KGZ` and `TCA`.
...
- Added `flag_draw_ops_test.dart` to enforce per-flag canvas command budgets recorded in `flag_draw_ops.json`.
- Added unit tests for `SimpleShieldPainter`, `ElementsPainter.calculateSize`, and `BadgeArtwork`.
- Added performance benchmarks and driver tests for scrolling flag lists and grids (`flag_list_perf_test.dart`).

CHORE

- Dart SDK bumped to v3.13.4.
- Added [material_ui](https://pub.dev/packages/material_ui) dependency.
