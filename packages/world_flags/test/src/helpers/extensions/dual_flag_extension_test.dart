...
void main() => group("DualFlagExtension", () {
  ...
  const original = DualFlag(item, map);
  ...
  test("copyWith preserves all values when no arguments provided", () {
    ...
    expect(copy.splitAngle, 30);
    expect(copy.clipSecondary, isFalse);
    expect(copy.clipBehavior, Clip.hardEdge);
  });
  ...
  test("copyWith updates splitAngle", () {
    final copy = original.copyWith(splitAngle: 60);
    expect(copy.splitAngle, 60);
  });

  test("copyWith updates clipSecondary", () {
    final copy = original.copyWith(clipSecondary: false);
    expect(copy.clipSecondary, isFalse);
  });

  test("copyWith updates clipBehavior", () {
    final copy = original.copyWith(clipBehavior: Clip.hardEdge);
    expect(copy.clipBehavior, Clip.hardEdge);
  });

  test("copyWith resets height to null with negative value", () {
    const withHeight = DualFlag(item, map, height: 100);
    final copy = withHeight.copyWith(height: -1);
    expect(copy.height, isNull);
  });

  test("copyWith resets width to null with negative value", () {
    const withWidth = DualFlag(item, map, width: 200);
  ...
  test("copyWith resets aspectRatio to null with negative value", () {
    const withAspect = DualFlag(item, map, aspectRatio: 1.5);
