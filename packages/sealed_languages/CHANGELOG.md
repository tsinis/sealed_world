...
Inherited from `l10n_languages` 2.1.0:
...
- Added a bundled agent skill (`skills/sealed-languages-core`), compliant with the [Agent Skills specification](https://agentskills.io), installable via `dart run skills@ get`.
...
- Finalized base classes (`NaturalLanguage`, `Script`, etc.) instances - sealed classes with private constructors, you can create your own instances via `LangCustom` and `ScriptCustom`, for example:

```dart
const custom = LangCustom(code: "XTL", codeShort: "XT"); // Custom language.
```

Changes from underlying L10N package:

Previously, localized strings were provided in mixed lowercase (e.g., "inglés", in Spanish for `ENG` code) and sentence case. They are now unified and provided in sentence case only (e.g., "Inglés", in Spanish for `ENG` code) to preserve capitalization context for proper nouns and ensure immediate compatibility with independent UI labels.
