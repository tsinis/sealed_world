...
void main() => group("TranslatedExtension", () {
  ...
  group("maybeTranslation", () {
    ...
    test("should use useLanguageFallback when no country code is provided", () {
      expect(
    ...
    test("should use useLanguageFallback when no script code is provided", () {
      expect(
        kongo.maybeTranslation(const BasicLocale(bosnia, script: ScriptCyrs())),
        isNotNull,
      );
      expect(
        kongo.maybeTranslation(
    ...
    test(
      ...
      () {
        final withoutCode = kongo
            .maybeTranslation(const BasicLocale(portuguese))
            ?.name;
    ...
    test(
      ...
      () {
        final withoutCode = kongo
            .maybeTranslation(const BasicLocale(bosnia))
            ?.name;
        final withCode = kongo
            .maybeTranslation(const BasicLocale(bosnia, script: ScriptCyrl()))
            ?.name;
  ...
  test("there should be always translations for specific languages", () {
    ...
    for (final value in NaturalLanguage.list) {
      for (final l10n in NaturalLanguage.list) {
        final hasTranslationForValue = value.maybeTranslation(
          BasicLocale(l10n),
        );
    ...
    final complete = sortedList.where(
      (item) => item.value == NaturalLanguage.list.length,
    );
