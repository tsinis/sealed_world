...
void main() => group("IsoStandardizedStringExtension", () {
  ...
  group("maybeMapIsoCode", () {
    const orElse = "orElse";
    test("should call short function when code length is equal to 2", () {
    ...
    test(
      ...
      () {
        for (final code in alphaRegularCodes.take(
          alphaRegularCodes.length - 1,
        )) {
          String result = code.maybeMapIsoCode(
            orElse: (_) => orElse,
            numeric: (_) => "numeric",
    ...
    test("should use maxLength with non default value for script codes", () {
    ...
    test("should use exactLength if maxLength is equal to minLength", () {
      final scriptCode = alphaRegularCodes.last;
