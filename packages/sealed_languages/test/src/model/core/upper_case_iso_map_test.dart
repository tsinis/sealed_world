...
void main() => group("$UpperCaseIsoMap", () {
  ...
  test("should return defaultValue when key is not present", () {
    ...
    const mapOther = UpperCaseIsoMap(defaultMap, defaultValue: defaultValue);
  ...
  group("standard map methods", () {
    ...
    test("entries", () {
      final expectedEntries = map.entries.map(
        (entry) => "${entry.key}: ${entry.value})",
      );
      final actualEntries = defaultMap.entries.map(
        (entry) => "${entry.key}: ${entry.value})",
      );
  ...
  group("unsupported errors", () {
    ...
    test("update", () {
      expect(() => map.update(eng, (value) => value), throwsUnsupportedError);
    });
